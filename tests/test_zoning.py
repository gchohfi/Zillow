...
def test_invalid_field_fallback_keeps_radius(tmp_path, monkeypatch):
    """Fallback para outFields=* continua usando o buffer do ponto."""
    distances = []

    def fake_get(url, params=None, headers=None, timeout=None, **kwargs):
        params = params or {}
        distances.append(params.get("distance"))
        if params.get("outFields") != "*":
            return _FakeResponse({"error": {"code": 400, "message": "Invalid field: DOR_UC"}})
        return _FakeResponse(_arcgis_payload({"DOR_UC": "0000"}))

    monkeypatch.setattr("src.zoning.requests.get", fake_get)

    zoning, _ = lookup_zoning(_listing(), _cfg(tmp_path, radius_m=30))

    assert zoning == "vacant residential"
    assert distances == [30, 30]
